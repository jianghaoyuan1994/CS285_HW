...
class MLPPolicy(BasePolicy):
    ...
    def define_forward_pass(self):
        if self.discrete:
            logits_na = build_mlp(self.observations_pl, output_size=self.ac_dim, scope='discrete_logits',
    ...
    def define_log_prob(self):
        if self.discrete:
            ...
            logits_na = self.parameters
            self.logprob_n = tfp.distributions.Categorical(logits=logits_na).log_prob(self.actions_pl)
        else:
    ...
    def get_action(self, obs):
        ...
        act = self.sess.run([self.sample_ac], feed_dict={self.observations_pl: observation})[0]
        return act
