...
class ACAgent(BaseAgent):
    ...
    def estimate_advantage(self, ob_no, next_ob_no, re_n, terminal_n):
        ...
        v_sn = self.critic.forward(next_ob_no)
        q = re_n + self.gamma * v_sn * (1-terminal_n)
        adv_n = q - v_s
    ...
    def train(self, ob_no, ac_na, re_n, next_ob_no, terminal_n):
        ...
        for i in range(self.agent_params["num_critic_updates_per_agent_update"]):
            c_loss = self.critic.update(ob_no=ob_no, next_ob_no=next_ob_no, re_n=re_n, terminal_n=terminal_n)
        ...
        for i in range(self.agent_params["num_actor_updates_per_agent_update"]):
            a_loss = self.actor.update(observations=ob_no, acs_na=ac_na, adv_n=advantage)
